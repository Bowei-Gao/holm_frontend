  ...
  methods: {
    async submitData() {
      const payload = {
        ...
        distances: this.input_distances.distances, // This should be dynamic based on your application's needs
        variable_cost_rates: this.input_distances.variable_cost_rates,
        weightings: this.input_distances.weightings
      };
    ...
    async submitDataPlanning() {
      const payload = {
        ...
        pickUpQuantities: this.customers.pickUpQuantities,
        x: this.customers.x,
...
<template>
  <zero v-if='page==0' @selected="page = $event; getAllCustomers(); getAllDepots();"></zero>
  ...
  <three v-if="page==3" @home="page = 0" @start-add-algorithm="page += 1; input_distances = $event; submitData();" :number_of_locations="result.number_of_locations" :number_of_customers="result.number_of_customers" :all_customers="all_customers"></three>
  <four v-if="page==4" @home="page = 0" :result_add_algorithm="result_add_algorithm"></four>
  <five v-if="page==5" @home="page = 0" @continue="page += 1; result = $event;"></five>
  <six v-if="page==6" @home="page = 0" @start-planning="page += 1; input_assignment=$event; submitDataAssignment();" :number_of_depots="result.number_of_depots" :number_of_customers="result.number_of_customers" :all_depots="all_depots" :all_customers="all_customers"></six>
  <seven v-if="page==7" @home="page = 0" :result_assignment="result_assignment"></seven>
  <eight v-if="page==8" @home="page = 0" @continue="page += 1; result = $event;" :all_depots="all_depots"></eight>
  <nine v-if="page==9" @home="page = 0" @continue="page += 1; customers = $event;" :number_of_customers="result.number_of_customers" :all_customers="all_customers"></nine>
